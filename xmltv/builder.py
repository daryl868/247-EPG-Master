...
def build_provider(provider, settings):
    ...
    for hour in range(0, days * 24, block):
        ...
        if info:
            desc = "\n".join([f"{i}. {ch.get('show','')} - {ch.get('subtitle','')}".strip() for i, ch in enumerate(provider["channels"], 1)])
            xml.extend([f'  <programme start="{xmltv_time(start, tz)}" stop="{xmltv_time(stop, tz)}" channel="{escape(info["id"])}">', "    <title>Current Lineup</title>", f'    <desc>{escape(desc)}</desc>', "  </programme>"])
        for ch in provider["channels"]:
            xml.extend([f'  <programme start="{xmltv_time(start, tz)}" stop="{xmltv_time(stop, tz)}" channel="{escape(ch["id"])}">', f'    <title>{escape(ch.get("show") or ch["name"])}</title>'])
            if ch.get("subtitle"):
                xml.append(f'    <sub-title>{escape(ch["subtitle"])}</sub-title>')
            xml.extend(["    <desc>24/7 channel</desc>", "  </programme>"])
    xml.append("</tv>")
    ...
    for item in all_channels:
        if item.get("stream_url"):
            m3u.append(f'#EXTINF:-1 tvg-id="{item["id"]}" tvg-name="{item["name"]}" tvg-logo="{logo}" group-title="{group}",{item["name"]}')
...
def build_all():
    ...
    out = ROOT / settings["generated_dir"]
    combined_xml = ['<?xml version="1.0" encoding="UTF-8"?>', '<tv generator-info-name="EPG Vision Studio">']
    combined_m3u = ["#EXTM3U"]
    for p in index["providers"]:
        ...
        combined_xml.extend([line for line in lines if not line.startswith("<?xml") and not line.startswith("<tv") and line.strip() != "</tv>"])
        m3u_path = out / provider["output_m3u"]
        combined_m3u.extend([line for line in m3u_path.read_text(encoding="utf-8").splitlines() if line != "#EXTM3U"])
